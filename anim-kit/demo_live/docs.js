 ...
 *
 * On a static host (demo_live/ deployed without the Node server) /api/prompts
 * is missing, so the generated prompts.json beside the page is the fallback.
 */
const api = new URL("/api/prompts", document.baseURI);
const apiStatic = new URL("prompts.json", document.baseURI);
...
/** /api/prompts first (npm run demo); prompts.json fallback on a static host. */
async function getPayload() {
  try {
    ...
    const res = await fetch(apiStatic);
    if (!res.ok) throw new Error(`HTTP ${res.status} from prompts.json`);
    return await res.json();
  }
}
  ...
  try {
    ...
    content.append(
      el(
        "p",
        "d-warn",
        `Catalogue unavailable — run \`npm run demo\` (serves /api/prompts) or keep prompts.json beside the page on a static host. (${err})`,
      ),
    );
  }
}
