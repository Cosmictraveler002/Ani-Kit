...
/* (GET /api/prompts from scripts/serve.mjs). On a static host       */
/* (demo_live/ without the Node server) the same payload ships as    */
/* prompts.json beside the page and is used as fallback.             */
/* ---------------------------------------------------------------- */
const promptApi = new URL("/api/prompts", document.baseURI);
const promptStatic = new URL("prompts.json", document.baseURI);
...
const loadPrompts = async () => {
  ...
  let data;
  try {
    data = await fetchJson(promptApi);
  } catch {
    data = await fetchJson(promptStatic); /* static host: no /api/prompts */
  }
  promptCache = {
    count: data.count,
